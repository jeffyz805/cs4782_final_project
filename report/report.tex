...
\textbf{Base LLM:} Qwen2.5-Coder-7B running locally via Ollama (fully open-source, free).
\textbf{Evaluation metrics:} fix success rate, first-attempt accuracy, token cost, backtrack rate.
No GPU required; runs on Apple Silicon via Ollama's Metal acceleration.
...
\textbf{Token cost.}
ToT uses roughly 7–13$\times$ more tokens than IO/CoT per problem. Since Qwen2.5-Coder runs
locally for free, this cost is purely in wall-clock time rather than dollars. CoT-SC at 66\%
offers a middle ground with significantly fewer LLM calls.
